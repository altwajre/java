...
https://www.safaribooksonline.com/library/view/scala-design-patterns/9781788471305/938e30b6-1982-4f37-b2f0-55c90c3e5fec.xhtml

The purpose of the mediator design pattern is to define an object that encapsulates how a set of other objects
interact with each other in order to promote loose coupling and allow us to vary class interactions independently.
 */
object Mediator {
...
/*
Student Ivan was notified with message: 'Design patterns in Scala are amazing!'
Student John was notified with message: 'Design patterns in Scala are amazing!'
Student Maria was notified with message: 'Design patterns in Scala are amazing!'
Student(John,25) is in groups: List(Group(Databases), Group(Scala design patterns))
Student(John,25) is in groups: List(Group(Scala design patterns))
Students in Group(Scala design patterns) are List(Student(Ivan,26), Student(John,25), Student(Maria,26))
 */
