...
public class ClientResource {
    ...
    public ContactView showContact(@QueryParam("id") int id) {
        Contact c = client.target("http://localhost:8080/contact/" + id).request()
                .get(Contact.class);
        String output = "ID: " + id
    ...
    public Response newContact(@QueryParam("firstName") String firstName, @QueryParam("lastName") String lastName, @QueryParam("phone") String phone) {
        Response response = client.target("http://localhost:8080/contact").request().post(Entity.entity(new Contact(0, firstName, lastName, phone), MediaType.APPLICATION_JSON));
        return response;
    }
    ...
    public Response updateContact(@QueryParam("id") int id, @QueryParam("firstName") String firstName, @QueryParam("lastName") String lastName, @QueryParam("phone") String phone) {
        Response response = client.target("http://localhost:8080/contact/" + id).request().put(Entity.entity(new Contact(id, firstName, lastName, phone), MediaType.APPLICATION_JSON));
        return response;
    }
