...
@Unroll
class BasicPipesSpec extends Specification {
    ...
    def "Trivial adder test"() {
        ...
        expect: "that is calculates the sum of two numbers"
    ...
    // 5.13 Trivial example of data pipes
    def "Valid images are PNG and JPEG files only"() {
        ...
        // Relationship of input and output parameters
        validator.isValidImageExtension(pictureFile) == validPicture

        where: "sample image names are"
        // All values of input parameter are inside a collection
        pictureFile << ["scenery.jpg", "house.jpeg", "car.png", "sky.tiff", "dance_bunny.gif"]
        // All values of output parameter are inside a collection
        validPicture << [true, true, true, false, false]

    }

    // 5.14 Using Groovy ranges as data generators
    ...
    // 5.15 Using Groovy combinations
